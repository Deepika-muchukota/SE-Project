...
function Protein() {
  ...
  useEffect(() => {
    const fetchMenuItems = async () => {
      try {
        ...
        const stallsResponse = await axios.get('http://localhost:5050/api/foodstalls');
        const halalStall = stallsResponse.data.foodstalls.find(stall => 
          stall.name.toLowerCase() === 'halal shack'
        );
        
        if (halalStall) {
          const menuResponse = await axios.get(`http://localhost:5050/api/foodstalls/${halalStall.id}/menu`);
  ...
  const handleItemClick = async (item) => {
    ...
    if (selectedItem && selectedItem.name === item.name) {
      ...
      if (cartItem) {
        try {
          await axios.delete(`http://localhost:5050/api/cart/delete/${user.id}/${item.id}`);
      ...
      try {
        ...
        await axios.post('http://localhost:5050/api/cart/add', cartItem);
