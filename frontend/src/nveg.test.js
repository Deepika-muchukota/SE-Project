...
describe("Nvegeterian Component", () => {
  const mockAddItemToCart = jest.fn();

  const mockCategories = [
    { name: "Pepperoni", price: "$12.99" },
    { name: "Chicken Tikka", price: "$11.99" },
    { name: "BBQ Chicken", price: "$13.49" },
  ];

  test("renders component correctly", () => {
    render(<Nvegeterian cart={{}} addItemToCart={mockAddItemToCart} />);

    expect(screen.getByText("Pepperoni - $12.99")).toBeTruthy();  // Alternative check
    expect(screen.getByText("Chicken Tikka - $11.99")).toBeTruthy();
    expect(screen.getByText("BBQ Chicken - $13.49")).toBeTruthy();
  });
